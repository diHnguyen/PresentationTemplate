...
\title[Shorter Version of the Title] % (optional, use only with long paper titles)
{Actual Title}

\author[LastName1, LastName2] % (optional, use only with multiple authors)
{Author 1, Author 2}
...
\begin{frame}[fragile]{Columns and Graphs}
We can write things outside of each column. 

We can also use \verb|\vspace{}| (\verb|\hspace{}|) to adjust the vertical (horizontal) position of the figure. 
\begin{figure}[tbh]\vspace*{1.5cm}\hspace*{-0.4cm}
    \centering
            ...
            \end{subfigure}
            \hfill
            \begin{subfigure}[b]{0.3\textwidth} %\vspace*{-1.2cm}\hspace*{-0.4cm}
            \begin{tikzpicture}[scale=0.8]
                \draw[thick,->] (0,0) -- (3,0) node[below] {\textbf{(1,3)}};
                \draw[thick,->] (0,0) -- (0,3) node[above] {\textbf{(2,4)}};
                \draw (0cm,1pt) -- (0cm,-1pt) node[below] {8};
